...
GO
--Tabla para registro de datos de los inquilinos
...
constraint fk_InquilinoLocalidad foreign key(codigolocalidad) references localidades(codigo),
constraint fk_InquilinoProvincia foreign key(CodigoProvincia) references provincia(codigo));
GO
-- Índice para búsquedas rápidas por CUIL
CREATE INDEX idx_Inquilino_CUIL ON Inquilino(CUIL);
GO
-- Índice para búsquedas rápidas por Nombre
CREATE INDEX idx_Inquilino_Nombre ON Inquilino(Nombre);
GO
-- Índice para búsquedas rápidas por Dirección
CREATE INDEX idx_Inquilino_Direccion ON Inquilino(Direccion);
GO
-- Índice para búsquedas rápidas por Teléfono
CREATE INDEX idx_Inquilino_Telefono ON Inquilino(Telefono);
GO
-- Índice para búsquedas rápidas por Email
CREATE INDEX idx_Inquilino_Email ON Inquilino(Email);
GO
-- Índice para búsquedas basadas en la relación con localidades
CREATE INDEX idx_Inquilino_Localidad ON Inquilino(CodigoLocalidad);
GO
-- Índice para búsquedas basadas en la relación con provincias
CREATE INDEX idx_Inquilino_Provincia ON Inquilino(CodigoProvincia);
GO
--Tabla para el registro de propietarios
...
CodigoLocalidad int,
CodigoProvincia int,
constraint pk_Propietario primary key(Codigo),
constraint fk_PropietarioLocalidad foreign key(codigolocalidad) references localidades(codigo),
constraint fk_PropietarioProvincia foreign key(CodigoProvincia) references provincia(codigo));
GO
-- Índice para búsquedas rápidas por CUIL
CREATE INDEX idx_Propietario_CUIL ON Propietario(CUIL);
GO
-- Índice para búsquedas rápidas por Nombre
CREATE INDEX idx_Propietario_Nombre ON Propietario(Nombre);
GO
-- Índice para búsquedas relacionadas con localidades
CREATE INDEX idx_Propietario_Localidad ON Propietario(CodigoLocalidad);
GO
-- Índice para búsquedas relacionadas con provincias
CREATE INDEX idx_Propietario_Provincia ON Propietario(CodigoProvincia);
GO
--Tabla que permite grabar los datos del Garante
...
CodigoLocalidad int,
CodigoProvincia int,
constraint pk_Garante primary key(Codigo),
constraint fk_GaranteLocalidad foreign key(codigolocalidad) references localidades(codigo),
constraint fk_GaranteProvincia foreign key(CodigoProvincia) references provincia(codigo));
GO
-- Índice para búsquedas rápidas por CUIL
CREATE INDEX idx_Garante_CUIL ON Garante(CUIL);

-- Índice para búsquedas rápidas por Nombre
CREATE INDEX idx_Garante_Nombre ON Garante(Nombre);

-- Índice para búsquedas por CódigoLocalidad
CREATE INDEX idx_Garante_Localidad ON Garante(CodigoLocalidad);

-- Índice para búsquedas por CódigoProvincia
CREATE INDEX idx_Garante_Provincia ON Garante(CodigoProvincia);

--Tabla que permite grabar los tipos de operaciones sobre las propiedades por ej: Venta, Alquiler, ETC.
Create table TipoOperacion(
Codigo int identity(1,1),
Descripcion varchar(50),
constraint pk_TipoOperacion primary key(Codigo));
GO

--tabla que permite grabar los tipos de propiedades, por ej: Duplex, Departamento, casa, etc.
Create table TipoPropiedad(
Codigo int identity(1,1),
Descripcion varchar(50),
constraint pk_TipoPropiedad primary key(Codigo));
GO

--Tabla que permite grabar los indices que hacen variar los valores de operaciones por ej: IPC o ICL en Argentina
create table Indice(
codigo int identity(1,1),
Descripcion varchar(50),
constraint pk_Indice primary key(codigo));
GO
--Tabla que permite Grabar los Valores fluctuantes de los porcentajes de variacion de indices
create table IndiceAjuste(
Codigo int identity(1,1),
CodigoIndice int,
Valor float,
Constraint pk_IndiceAjuste primary key(codigo),
Constraint fk_IndiceAJusteIndice foreign key(CodigoIndice) references Indice(Codigo));
GO
--Tabla que permite grabar las diferentes monedas que maneja la empresa
Create Table Moneda(
Codigo varchar(10),
Descripcion varchar(50),
Constraint pk_Moneda primary key(Codigo));
Go
--Tabla que permite grabar los datos principales de las propiedades
...
Constraint PK_Propiedad primary key(Codigo),
Constraint FK_PropiedadPropietario Foreign Key(CodigoPropietario) references Propietario(Codigo),
Constraint fk_PropiedadTIpoPropiedad foreign key(CodigoTipoPropiedad) references TipoPropiedad(codigo),
CONSTRAINT chk_CantidadAmbientes CHECK (CantidadAmbientes >= 0),
CONSTRAINT chk_CantidadBaños CHECK (CantidadBaños >= 0),
CONSTRAINT chk_CantidadDormitorios CHECK (CantidadDormitorios >= 0));
GO
-- Índice para búsquedas por propietario
CREATE INDEX idx_Propiedad_Propietario ON Propiedad(CodigoPropietario);
GO
-- Índice para búsquedas por tipo de propiedad
CREATE INDEX idx_Propiedad_TipoPropiedad ON Propiedad(CodigoTipoPropiedad);
GO
-- Índice para búsquedas por dirección
CREATE INDEX idx_Propiedad_Direccion ON Propiedad(Direccion);
GO
--Tabla que permite grabar las operaciones permitidas sobre una misma propiedad
Create Table PropiedadTipoOperacion(
Codigo int identity(1,1),
CodigoPropiedad int,
CodigoTipoOperacion int,
Precio Float,
Constraint pk_PropiedadTIpoOperacion primary key(COdigo),
Constraint fk_PropiedadTipoOperacionPropiedad foreign key(CodigoPropiedad) references Propiedad(Codigo),
Constraint fk_PropiedadTIpoOperacionTipoOperacion foreign key(CodigoTipoOperacion) references TipoOperacion(Codigo),
CONSTRAINT chk_Precio_Positive CHECK (Precio >= 0));
GO
-- Índice para búsquedas rápidas por CódigoPropiedad
CREATE INDEX idx_PropiedadTipoOperacion_Propiedad ON PropiedadTipoOperacion(CodigoPropiedad);
GO
-- Índice para búsquedas rápidas por CódigoTipoOperacion
CREATE INDEX idx_PropiedadTipoOperacion_TipoOperacion ON PropiedadTipoOperacion(CodigoTipoOperacion);
GO
-- Índice compuesto para búsquedas por propiedad y tipo de operación
CREATE INDEX idx_PropiedadTipoOperacion_Compuesto ON PropiedadTipoOperacion(CodigoPropiedad, CodigoTipoOperacion);
GO
--tabla que permite grabar los periodos actualizacion de los montos
...
GO
--Tabla que permite grabar los datos del contrato Alquiler
...
CodigoPropietario int,
CodigoInquilino int,
PeriodoActualizacion varchar(50),
DiaVencimiento int,
CodigoPeriodo int,
Constraint pk_ContratoAlquiler primary key(Codigo),
Constraint pk_ContratoAlquilerPropiedad foreign Key(CodigoPropiedad) references Propiedad(Codigo),
Constraint pk_ContratoAlquilerPropietario foreign key(CodigoPropietario) references Propietario(Codigo),
Constraint pk_ContratoAlquilerInquilino foreign key(CodigoInquilino) references inquilino(Codigo),
CONSTRAINT chk_Fechas_Validas CHECK (FechaVencimiento > FechaInicio),
CONSTRAINT chk_DiaVencimiento CHECK (DiaVencimiento BETWEEN 1 AND 31));
GO
-- Índice para búsquedas rápidas por propiedad
CREATE INDEX idx_ContratoAlquiler_Propiedad ON ContratoAlquiler(CodigoPropiedad);
GO
-- Índice para búsquedas rápidas por propietario
CREATE INDEX idx_ContratoAlquiler_Propietario ON ContratoAlquiler(CodigoPropietario);
GO
-- Índice para búsquedas rápidas por inquilino
CREATE INDEX idx_ContratoAlquiler_Inquilino ON ContratoAlquiler(CodigoInquilino);
GO
...
Insert into Periodo (Descripcion,Dias) values
('Mensual',30),
('Bimestral',60),
('Trimestral',90),
('Semestral',180),
...
--VISTA para datos de los inquilinos
CREATE VIEW VistaInquilinoCompleto AS
SELECT 
    i.Codigo,
    i.CUIL,
    i.Nombre,
    ...
    i.Email,
    i.CodigoLocalidad,
    l.localidad AS NombreLocalidad,
    i.CodigoProvincia,
    p.provincia AS NombreProvincia
FROM 
    Inquilino i
INNER JOIN 
    localidades l ON i.CodigoLocalidad = l.Codigo
INNER JOIN 
...
SELECT 
    p.Codigo AS CodigoPropiedad,
    p.Descripcion,
    p.Direccion,
    p.CodigoIndice,
    p.CantidadAmbientes,
    p.CantidadBaños,
    p.CantidadDormitorios,
    p.AptoProfesional,
    p.TieneExpensas,
    p.ValorExpensas,
    p.Tamanio,
    p.AceptaMascotas,
    tp.Descripcion AS TipoPropiedad,
    pr.Nombre AS NombrePropietario,
    pr.Telefono AS TelefonoPropietario,
    pr.Email AS EmailPropietario
FROM 
    Propiedad p
INNER JOIN 
    Propietario pr ON p.CodigoPropietario = pr.Codigo
INNER JOIN 
    TipoPropiedad tp ON p.CodigoTipoPropiedad = tp.Codigo;